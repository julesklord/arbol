...
func parseFlags() (bool, bool, string, string, string, string, string, bool, int, string, bool, int) {
	...
	for _, arg := range os.Args[1:] {
		if arg == "--no-ascii" {
			...
			fmt.Println("Options:")
			fmt.Println("  -h, --help                  Display usage instructions.")
			fmt.Println("  -v, --version               Print the installed version and exit.")
			fmt.Println("      --no-ascii              Omit the system ASCII logo.")
			fmt.Println("      --minimal               Skip extended plugins and display a single info card.")
			fmt.Println("      --noframe               Omit the box borders and print layout side-by-side using spaces.")
			fmt.Println("      --output=FORMAT         Serialize output: json, xml, or txt.")
			fmt.Println("      --logo=MODE             Control ASCII logo style: simple or banner.")
			fmt.Println("      --theme=NAME            Color theme: default, catppuccin, catppuccin-mocha, catppuccin-latte, dracula, nord, tokyonight, gruvbox, everforest, monokai, rose-pine, solarized.")
			fmt.Println("      --bar-style=STYLE       Progress bar style: block, braille, gradient, dot.")
			fmt.Println("      --tree-style=STYLE      Tree connector style: default, rounded, heavy, double, ascii, dotted.")
			fmt.Println("      --sparkline[=WIDTH]     Enable inline sparklines for resources (default width: 20).")
			fmt.Println("      --sparkline-style=STYLE Sparkline style: block, braille, dots.")
			fmt.Println("      --live[=MS]             Live updating mode with interval in ms (default: 1000).")
			fmt.Println("      --plugins-dir=PATH      Override plugin discovery directory.")
			os.Exit(0)
		} else {
